...
const { toMediaUrl } = require('./media-url.js');
const { outRoot } = require('./paths');
const { probeDuration } = require('./ffmpeg-util');
const {
  normalizeCrop, normalizeCam, normalizeExportRate, normalizeMusic, normalizeCaptions, normalizeVertical,
} = require('../domain/clip-ops.ts');
...
function normalizeManifest(doc, takeId, duration) {
  ...
  const captions = normalizeCaptions(doc.captions);
  const exportRate = normalizeExportRate(doc.exportRate);
  const music = normalizeMusic(doc.music);
  const vertical = normalizeVertical(doc.vertical);
  return {
    version: 1,
    takeId,
    source: doc.source || 'screen.mp4',
...
function mediaUrls(takeId) {
  ...
  for (const name of ['screen.mp4', 'cam.mp4', 'audio.mp3', FINAL_NAME]) {
    ...
    if (fs.existsSync(p)) map[name] = toMediaUrl(p);
  }
