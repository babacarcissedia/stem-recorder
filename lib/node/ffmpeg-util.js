...
function renderCanvasDimensions({ sourceDimensions, crop = null, verticalPreset = null }) {
  if (verticalPreset) {
    return {
      width: verticalPreset.scale.width,
      height: verticalPreset.scale.height,
    };
  }
  if (!sourceDimensions) return null;
  return {
    width: crop ? Math.floor((sourceDimensions.width * crop.w) / 2) * 2 : sourceDimensions.width,
    height: crop ? Math.floor((sourceDimensions.height * crop.h) / 2) * 2 : sourceDimensions.height,
  };
}
...
function buildClipRenderPlan({
  srcPath, dialoguePath = null, cam = null, crop = null, subtitles = null, rate = null, verticalPreset = null, pip = null,
}) {
  ...
  if (cam) {
    if (!pip) throw new Error('cam clip render plan requires a PiP layout');
    filterArgs = [
      '-filter_complex', pipFilterGraph({
        crop, cam, ...pip, subtitlesPath: subtitles, rate, verticalPreset,
        camInputIndex: mediaInput.camInputIndex,
      }),
      '-map', '[v]', ...mediaInput.audioMapArgs,
    ];
  }
  return Object.freeze({ inputArgs: mediaInput.inputArgs, filterArgs });
}

function resolveTakeLocalDialoguePath(takeDir, dialogueSource) {
  if (typeof dialogueSource !== 'string' || dialogueSource.length === 0 || dialogueSource.includes('\0')) {
    throw new Error('selected dialogue source path must be a non-empty string');
  }
  const realTakeDir = fs.realpathSync(takeDir);
  const dialoguePath = path.resolve(realTakeDir, dialogueSource);
  const relative = path.relative(realTakeDir, dialoguePath);
  if (relative === '' || relative === '..' || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
    throw new Error(`selected dialogue source is outside take directory: ${dialogueSource}`);
  }
  if (!fs.existsSync(dialoguePath)) {
    throw new Error(`selected dialogue source is missing: ${dialogueSource}`);
  }
  if (!fs.statSync(dialoguePath).isFile()) {
    throw new Error(`selected dialogue source is not a file: ${dialogueSource}`);
  }
  const realDialoguePath = fs.realpathSync(dialoguePath);
  const realRelative = path.relative(realTakeDir, realDialoguePath);
  if (realRelative === '..' || realRelative.startsWith(`..${path.sep}`) || path.isAbsolute(realRelative)) {
    throw new Error(`selected dialogue source is outside take directory: ${dialogueSource}`);
  }
  return realDialoguePath;
}
...
function clipCacheKey(plan, clip) {
  ...
  const payload = {
    ...
    captions: !isFreeze && plan.captions ? plan.captions : null,
    cam: plan.cam ? {
      path: plan.cam.path,
      stamp: plan.camStamp,
...
async function applyClips(srcPath, clips, outPath, workDir, opts = {}) {
  ...
  const needsRenderDimensions = cam || vertical || (captions && captions.style === 'karaoke');
  let dim = null;
  if (needsRenderDimensions) {
    dim = probeDimensions(srcPath);
    if (!dim) throw new Error('cannot probe screen dimensions for cam PiP / vertical export / karaoke captions');
  }
  const baseDimensions = renderCanvasDimensions({ sourceDimensions: dim, crop: first });
  const baseWidth = baseDimensions ? baseDimensions.width : null;
  const baseHeight = baseDimensions ? baseDimensions.height : null;
  ...
  if (vertical) {
    ...
    verticalPreset = buildVerticalPreset({
      ...
      cam: camDim,
      pip: vertical.pip || {},
      autozoom: vertical.autozoom,
    });
  }

  const renderDimensions = renderCanvasDimensions({ sourceDimensions: dim, crop: first, verticalPreset });
  let captionsResolved = null;
  if (captions && captions.style === 'karaoke') {
    if (!renderDimensions) throw new Error('cannot resolve karaoke captions without render dimensions');
    const options = {
      ...(captions.options || {}),
      resolutionX: renderDimensions.width,
      resolutionY: renderDimensions.height,
    };
    captionsResolved = {
      path: resolveCaptionsPath(captions.editDir, options),
      style: 'karaoke',
      options,
    };
  } else if (captions && captions.style === 'segment') {
    captionsResolved = { path: captions.vttPath, style: 'segment' };
  }

  const hasFreeze = clips.some((c) => c.freeze);
  const withSilence = hasFreeze && dialogue != null;
  ...
  function buildPlan(subtitles) {
    ...
    const inputArgs = renderPlan.inputArgs;
    let filterArgs = renderPlan.filterArgs;
  ...
  const subtitles = captionsResolved ? captionsResolved.path : (opts.subtitles || null);
