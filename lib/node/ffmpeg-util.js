...
const { resolveAudioRoute } = require('../domain/source.ts');
...
function resolveAudioSource({ srcPath, camPath, micPath }) {
  const candidates = [{
    id: 'source',
    path: srcPath,
    label: path.basename(srcPath),
    kind: 'video',
    hasAudio: probeHasAudio(srcPath),
    present: fs.existsSync(srcPath),
  }];
  if (camPath && path.resolve(camPath) !== path.resolve(srcPath)) {
    candidates.push({
      id: 'cam',
      path: camPath,
      label: path.basename(camPath),
      kind: 'video',
      hasAudio: fs.existsSync(camPath) && probeHasAudio(camPath),
      present: fs.existsSync(camPath),
    });
  }
  if (micPath && path.resolve(micPath) !== path.resolve(srcPath)) {
    candidates.push({
      id: 'mic',
      path: micPath,
      label: path.basename(micPath),
      kind: 'audio',
      hasAudio: fs.existsSync(micPath) && probeHasAudio(micPath),
      present: fs.existsSync(micPath),
    });
  }
  const id = resolveAudioRoute(candidates);
  if (!id) return null;
  const picked = candidates.find((candidate) => candidate.id === id);
...
function clipCacheKey(plan, clip) {
  ...
  const payload = {
    ...
    audio: plan.audioSource ? { path: plan.audioSource.path, stamp: plan.audioStamp || null } : null,
  };
...
async function applyClips(srcPath, clips, outPath, workDir, opts = {}) {
  ...
  const audioSource = resolveAudioSource({
    srcPath,
    camPath: cam ? cam.path : (stems.cam || null),
    micPath: stems.mic || null,
  });
  const audioStamp = audioSource ? fileStamp(audioSource.path) : null;

  const hasFreeze = clips.some((c) => c.freeze);
  const withSilence = hasFreeze && audioSource !== null;
  ...
  function buildPlan(subtitles) {
    ...
    let audioIndex = -1;
    if (audioSource) {
      audioIndex = inputPaths.findIndex((candidate) => path.resolve(candidate) === path.resolve(audioSource.path));
      if (audioIndex < 0) {
        inputPaths.push(audioSource.path);
        audioIndex = inputPaths.length - 1;
      }
      if (cam) filterArgs = [...filterArgs, '-map', `${audioIndex}:a:0`];
      else if (inputPaths.length > 1) filterArgs = [...filterArgs, '-map', '0:v:0', '-map', `${audioIndex}:a:0`];
    }
