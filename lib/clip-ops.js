...
(function (root, factory) {
  ...
  /**
   * Take-level cam settings ({ mirror }). Cam-only: screen stays unmirrored.
   * "No settings" is stored as an absent key, like crop.
   */
  function normalizeCam(cam) {
    if (!cam || typeof cam !== 'object') return null;
    return cam.mirror === true ? { mirror: true } : null;
  }
