 ...
 */
function pipFilterGraph({ crop, cam, pipWidth, margin, layout }) {
  ...
  return [
    ...
    `[base][pip]overlay=${pipOverlayPosition(layout, margin)}[v]`,
  ].join(';');
}
...
async function applyClips(srcPath, clips, outPath, workDir, opts = {}) {
  ...
  if (cam) {
    ...
    const pipWidth = Math.max(2, Math.floor((baseWidth * widthFrac) / 2) * 2);
    const margin = Math.max(PIP_MIN_MARGIN, Math.round(baseWidth * PIP_MARGIN_FRAC));
    inputArgs = ['-i', srcPath, '-i', cam.path];
    filterArgs = [
      '-filter_complex', pipFilterGraph({ crop: first, cam, pipWidth, margin, layout }),
      '-map', '[v]', '-map', '0:a?',
    ];
  }
