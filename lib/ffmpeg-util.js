...
function freezeArgs(plan, clip) {
  ...
  if (plan.cam) {
    args.push('-filter_complex', pipFilterGraph({ crop: plan.crop, cam: plan.cam, ...plan.pip, freezeDur: holdDuration }), '-map', '[v]');
    if (plan.withSilence) args.push('-map', '2:a');
  } else {
    args.push('-vf', [plan.crop ? cropFilter(plan.crop) : null, freezeStillChain(holdDuration)].filter(Boolean).join(','), '-map', '0:v:0');
...
async function applyClips(srcPath, clips, outPath, workDir, opts = {}) {
  ...
  const plan = Object.freeze({
    ...
    camStamp: cam ? fileStamp(cam.path) : null,
    cacheDir,
    expectedSec,
  });
  ...
  const renderTarget = plan.music ? path.join(workDir, 'pre-music.mp4') : outPath;
  ...
  if (clips.length === 1) {
    ...
    if (plan.music) await mixMusicBed(renderTarget);
    verifyOutputDuration(outPath, plan.expectedSec);
    return;
  }
