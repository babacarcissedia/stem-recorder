...
const { expectedOutputDuration, durationTolerance } = require('./apply-duration');
const { buildKaraokeAss } = require('./captions');
const { buildVerticalPreset } = require('./export-presets');
...
/**
 * ffmpeg crop+scale filter chain from a lib/export-presets.js
 * buildVerticalPreset() result. crop/scale are absolute pixel values —
 * export-presets.js already resolved them against the known post-manual-
 * crop base dimensions — unlike cropFilter()'s normalized 0–1 rect used for
 * the editor's per-clip crop.
 */
function verticalCropScaleFilter(preset) {
  const { crop, scale } = preset;
...
/**
 * Resolves the caption artifact Apply should burn for a take: a per-word
 * karaoke ASS document (lib/captions.js) built from asr.json's word-level
 * timings when they're present, falling back to the existing segment-level
 * VTT when they're not (an older asr.json, or a provider that never emitted
 * `words`). Additive — a caller that still passes a VTT path straight to
 * applyClips({ subtitles }) is unaffected by this function existing.
 *
 * The ASS is written under edit/.cache/, not directly in edit/ alongside
 * captions.vtt. Unlike the VTT (which the transcript panel's double-click
 * cue edit rewrites in place — user-editable state), the ASS is a pure
 * derived rendering of asr.json + styleOptions with nothing to edit by hand,
 * so it follows the same "regenerable, derived, per-take" contract as the
 * filmstrip/waveform caches already under edit/.cache/ rather than the
 * first-class edit/ artifacts. buildKaraokeAss is synchronous pure JS, so it
 * is unconditionally rewritten here instead of key-cached.
 *
 * @param {string} editDir the take's edit/ directory (asr.json, captions.vtt live here)
 * @param {object} [styleOptions] forwarded to buildKaraokeAss — wordsPerCue, fontSize, verticalPosition, etc.
 * @returns {string|null} absolute path to the subtitles file Apply should burn, or null when neither artifact exists
 */
function resolveCaptionsPath(editDir, styleOptions = {}) {
  ...
  if (fs.existsSync(asrPath)) {
    ...
    try {
      ...
      words = null; // malformed asr.json — fall back to VTT below
    }
    if (words) {
 ...
 *
 * verticalPreset (a lib/export-presets.js buildVerticalPreset() result)
 * inserts the 9:16 crop+scale stage into the base chain right after the
 * manual crop and BEFORE the cam overlay: the preset's pip rect is defined
 * in target/output pixel space, so the frame has to already be at target
 * dimensions by the time overlay reads it. It also overrides the cam's
 * scale width and overlay position with the preset's cam-aspect-correct
 * pip.w/x/y (absolute pixels) instead of pipWidth/layout/margin. Caption
 * burn stays LAST in the base-chain-adjacent stages (after overlay, before
 * speed) so cues draw on the final vertical frame at its native resolution
 * — burning before the vertical scale would let the crop clip cue text or
 * the scale blur it.
 */
function pipFilterGraph({
  crop, cam, pipWidth, margin, layout, freezeDur, subtitlesPath, rate, verticalPreset,
}) {
  const baseStages = [
 ...
 * timeline.
 *
 * opts.vertical = { target?, pip? } requests a 9:16 (or other) vertical
 * export via lib/export-presets.js buildVerticalPreset(): the manual crop
 * (if any) still applies first, then the vertical crop+scale composes right
 * after it and before the cam PiP overlay — the preset's pip rect is
 * computed in target pixel space, so the frame must already be at target
 * size when overlay reads it. See pipFilterGraph's verticalPreset doc for
 * the cam-side details. Geometry only — duration is unaffected, so
 * verifyOutputDuration's expectations are untouched by this option.
 */
async function applyClips(srcPath, clips, outPath, workDir, opts = {}) {
...

  {
    ...
    if (vf.length) filterArgs = ['-vf', vf.join(',')];
  }
  let pip = null;
  if (cam) {
  ...
  // Audio keeps pace with the sped video; -af only touches mapped source
  // audio (0:a?), which stays outside any -filter_complex graph.
  if (rate) filterArgs = [...filterArgs, '-af', atempoChain(rate).join(',')];
  ...
  function freezeArgs(clip, outFile) {
    ...
    const srcDur = Number(clip.out) - start;
    if (!(srcDur > 0)) throw new Error('freeze segment needs a positive duration');
    // A freeze hold is wall-clock, not a source range: at export rate r it
    // renders r× shorter/longer directly instead of via setpts/atempo.
    const dur = rate ? Math.round((srcDur / rate) * 1000) / 1000 : srcDur;
    ...
    if (cam) {
      args.push(
        '-filter_complex',
        pipFilterGraph({
          crop: first, cam, ...pip, freezeDur: dur, verticalPreset,
        }),
        '-map', '[v]',
      );
  ...
  // Per-clip render cache (A2), co-located with the other per-take caches
  // (edit/.cache/... — see ARCHITECTURE.md). Keyed on every parameter that
  // actually reaches clipArgs/freezeArgs PLUS each source file's mtime+size
  // stamp (the media-cache.js pattern) — a source edit or any take-level
  // setting change produces a different key, so a stale render can never be
  // served under a key that also matches the current inputs. This is what
  // makes the cache safe: it never overrides the A1 duration check above,
  // it just skips ffmpeg when the key proves the output would be identical.
  const clipCacheDir = path.join(workDir, '..', '.cache', 'clip-parts');
  ...
  function clipCacheKey(clip) {
    ...
    const payload = {
      ...
      freeze: isFreeze,
      crop: first || null,
      // Freeze segments never burn captions (see freezeArgs) or apply
      // subtitles filters — keying on that keeps a caption-only edit from
      // invalidating cached freeze segments it can't have affected.
      subtitles: !isFreeze && subtitles ? { path: subtitles, stamp: fileStamp(subtitles) } : null,
      cam: cam ? {
        path: cam.path,
        stamp: fileStamp(cam.path),
        mirror: !!cam.mirror,
        rotate: cam.rotate || 0,
        layout: cam.layout || null,
      } : null,
      rate: rate || null,
      // Freeze segments DO apply the vertical crop/scale (see freezeArgs) —
      // unlike captions/subtitles it's not skipped there — so this keys on
      // it unconditionally, freeze or not.
      vertical: vertical ? { target: vertical.target || null, pip: vertical.pip || null } : null,
      srcStamp: fileStamp(srcPath),
    };
    return crypto.createHash('sha1').update(JSON.stringify(payload)).digest('hex');
  }
  async function renderClipPart(clip, destPath) {
    const cacheFile = path.join(clipCacheDir, `${clipCacheKey(clip)}.mp4`);
    if (fs.existsSync(cacheFile)) {
      fs.copyFileSync(cacheFile, destPath);
      return;
    }
    await runFfmpeg(ffmpeg, clipArgs(clip, destPath));
    fs.mkdirSync(clipCacheDir, { recursive: true });
    // Populate via tmp+rename so a process killed mid-copy never leaves a
    // truncated file sitting under a valid-looking cache key.
    const tmp = path.join(clipCacheDir, `.tmp-${process.pid}-${Date.now()}`);
    fs.copyFileSync(destPath, tmp);
    fs.renameSync(tmp, cacheFile);
  }

  // With a music bed, render the (possibly sped) cut to a scratch file first,
  // then score it in one mixing pass into outPath.
  const renderTarget = music ? path.join(workDir, 'pre-music.mp4') : outPath;

  async function mixMusicBed(videoPath) {
    await runFfmpeg(ffmpeg, [
      '-hide_banner', '-y',
      '-i', videoPath,
      '-stream_loop', '-1', '-i', music.path,
      '-filter_complex', musicMixGraph({ gainDb: music.gainDb, baseHasAudio: probeHasAudio(videoPath) }),
      '-map', '0:v:0', '-c:v', 'copy',
      '-map', '[a]', '-c:a', 'aac',
      '-shortest',
      '-movflags', '+faststart',
  ...
  // The clips[] the model expects the render to produce — computed up front
  // so a probe failure or mismatch below can name both numbers.
  ...
  if (clips.length === 1) {
    await renderClipPart(clips[0], renderTarget);
    if (music) await mixMusicBed(renderTarget);
    verifyOutputDuration(outPath, expectedSec);
    return;
  }
  ...
  for (let i = 0; i < clips.length; i += 1) {
    ...
    await renderClipPart(clips[i], part);
    parts.push(part);
  }
