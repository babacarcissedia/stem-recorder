 ...
 */
function pipFilterGraph({ crop, cam, pipWidth, margin, layout, freezeDur, subtitlesPath, rate }) {
  ...
  const lines = [
    ...
    `[base][pip]overlay=${pipOverlayPosition(layout, margin)}${stages.length ? '[ov]' : '[v]'}`,
  ];
...
async function applyClips(srcPath, clips, outPath, workDir, opts = {}) {
  ...
  if (cam) {
    ...
    filterArgs = [
      '-filter_complex', pipFilterGraph({ crop: first, cam, ...pip, subtitlesPath: subtitles, rate }),
      '-map', '[v]', '-map', '0:a?',
    ];
  }
