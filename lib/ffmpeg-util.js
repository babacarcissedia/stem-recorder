 ...
 *
 * subtitlesPath (Edit-T2d) appends a caption burn-in stage last — after
 * crop, overlay and freeze — so cues draw on the composed frame.
 */
function pipFilterGraph({ crop, cam, pipWidth, margin, layout, freezeDur, subtitlesPath }) {
...
async function applyClips(srcPath, clips, outPath, workDir, opts = {}) {
  ...
  let filterArgs = [];
  {
    const vf = [first ? cropFilter(first) : null, subtitles ? subtitlesFilter(subtitles) : null].filter(Boolean);
  ...
  if (cam) {
    ...
    filterArgs = [
      '-filter_complex', pipFilterGraph({ crop: first, cam, ...pip, subtitlesPath: subtitles }),
      '-map', '[v]', '-map', '0:a?',
    ];
  }
